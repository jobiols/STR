...
class curso_curso(osv.osv):
    ...
    def generate_doc_curso(self, cr, uid, ids, context=None):
        for curso in self.browse(cr, uid, ids, context=context):
            ...
            records = reg_pool.search(cr, uid, [('curso_id', '=', curso.id), ('state', '=', 'confirm')])
            for reg in reg_pool.browse(cr, uid, records, context=context):
                alumnas.append({'name': reg.partner_id.name, 'credit': reg.partner_id.credit})

            clases = []
            lect_pool = self.pool.get('curso.lecture')
            records = lect_pool.search(cr, uid, [('curso_id', '=', curso.id)], order="date")
            for lect in lect_pool.browse(cr, uid, records, context=context):
                d = {
                    'fecha': datetime.strftime(datetime.strptime(lect.date, "%Y-%m-%d"), "%d/%m/%y"),
                    'desc': lect.desc,
                }
                clases.append(d)
    ...
    def button_cancel(self, cr, uid, ids, context=None):
        ...
        for curso_reg in registration.browse(cr, uid, reg_ids, context=context):
            if curso_reg.state != 'cancel':
                raise osv.except_osv('Error!', u"Para cancelar el curso debe cancelar todas las alumnas.")
    ...
    def check_registration_limits_before(self, cr, uid, ids, no_of_registration, context=None):
        for curso in self.browse(cr, uid, ids, context=context):
            ...
            if available_seats and no_of_registration > available_seats:
                raise osv.except_osv(('Cuidado!'), (u"Solo hay %d vacantes disponnibles!") % (available_seats))
            elif available_seats == 0:
                raise osv.except_osv(('Cuidado!'), (u"No Hay mas vacantes en este curso!"))
    ...
    def button_confirm(self, cr, uid, ids, context=None):
        ...
        for curso in self.browse(cr, uid, ids, context=context):
            if not (curso.date_begin):
                raise osv.except_osv(('Error!'), (u"No se puede confirmar el curso porque no tiene fecha de inicio."))

            if not ((curso.schedule_1) and (curso.weekday_1)):
                raise osv.except_osv(('Error!'), (u"No se puede confirmar el curso porque no horario y día asignados."))
    ...
    def generate_lectures(self, cr, uid, ids, context=None):
        ...
        if (operator.mod(tot_hs_lecture, hs_lecture) != 0):
            raise osv.except_osv(('Error!'), (u"la cantidad de horas catedra no es divisible por las horas de clase!."))
    ...
    def _get_name(self, cr, uid, ids, fields, args, context=None):
        ...
        for curso in self.browse(cr, uid, ids, context=context):
            ...
            name = "[{}] {} {}/{} ({:0>2d}:{:0>2d} {:0>2d}:{:0>2d}) - {}".format(
                format_instance(curso.product.default_code, curso.instance),  # Codigo de producto, Nro de instancia
    ...
    _columns = {
        ...
        'product': fields.many2one('product.product', 'Producto', required=True, domain="[('tot_hs_lecture','!=','0')]",
                                   readonly=True, states={'draft': [('readonly', False)]}),
        ...
        'country_id': fields.related('address_id', 'country_id', type='many2one', relation='res.country',
                                     string='Country',
        ...
        # Calculated fields
        'curso_instance': fields.function(_get_instance, fnct_search=None, string='Instancia del curso', method=True,
                                          store=False, type='char'),
        'name': fields.function(_get_name, fnct_search=None, string='Nombre del curso', method=True, store=True,
                                type='char'),
        'no_lectures': fields.function(_get_no_lectures, string='Clases', method=True, type='char',
        ...
        'register_avail': fields.function(_get_register, string='Vacantes', multi='register_numbers', type='integer'),
        'register_prospect': fields.function(_get_register, string='Interesadas',
                                             ...
                                             multi='register_numbers'),
        'register_attended': fields.function(_get_register, string='Egresadas', multi='register_numbers',
                                             help=u"Cantidad de alumnas que termino el curso con exito."),

        # Related fields
        'tot_hs_lecture': fields.related('product', 'tot_hs_lecture', type='float', string='Tot Hs', readonly=True),
        'list_price': fields.related('product', 'list_price', type='float', string='Cuota', readonly=True),
        'hs_lecture': fields.related('product', 'hs_lecture', type='float', string='Hs', readonly=True),
        'default_code': fields.related('product', 'default_code', type='char', string=u'Código', readonly=True),
        'no_quotes': fields.related('product', 'no_quotes', type='integer', string='#cuotas', readonly=True),
    ...
    _defaults = {
        'state': 'draft',
        'company_id': lambda self, cr, uid, c: self.pool.get('res.company')._company_default_get(cr, uid, 'curso.curso',
                                                                                                 context=c),
    ...
    def subscribe_to_curso(self, cr, uid, ids, context=None):
        ...
        if not curr_reg_ids:
            curr_reg_ids = [register_pool.create(cr, SUPERUSER_ID,
                                                 {'curso_id': ids[0], 'email': user.email, 'name': user.name,
                                                  'user_id': user.id, 'nb_register': num_of_seats})]
        else:
...
class curso_registration(osv.osv):
    ...
    _columns = {
        ...
        'reply_to': fields.related('curso_id', 'reply_to', string='Reply-to Email', type='char', size=128,
                                   readonly=True, ),
        'log_ids': fields.one2many('mail.message', 'res_id', 'Logs', domain=[('model', '=', _name)]),
        'curso_begin_date': fields.related('curso_id', 'date_begin', type='date', string="Inicio", readonly=True),
        'curso_begin_day': fields.function(_get_weekday, string='Dia', method=True, store=False, type='char'),
        'user_id': fields.many2one('res.users', 'User', states={'done': [('readonly', True)]}),
        'company_id': fields.related('curso_id', 'company_id', type='many2one', relation='res.company',
        ...
        'email': fields.related('partner_id', 'email', string='Email', type='char', size=128, readonly=True),
        'phone': fields.related('partner_id', 'mobile', string='Telefono', type='char', size=128, readonly=True),
    ...
    def button_reg_done(self, cr, uid, ids, context=None):
        ...
        for registration in self.browse(cr, uid, ids, context=context):
            ...
            records = register_pool.search(cr, uid, [('registration_id', '=', registration.id), ('list_price', '=', 0)])
            if len(records) != 0:
                raise osv.except_osv(('Error!'), (
    ...
    def button_reg_cancel(self, cr, uid, ids, context=None, *args):
        ...
        for registration in self.browse(cr, uid, ids, context=context):
            id = registration.id
            register_pool = self.pool.get('curso.quota')
            records = register_pool.search(cr, uid, [('invoice_id', '=', None), ('registration_id', '=', id)])
